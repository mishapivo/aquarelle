...
/* ------------------------------------ */
if ( ! function_exists( 'enspire_hex2rgb' ) ) {

	function enspire_hex2rgb( $hex, $array=false ) {
		$hex = str_replace("#", "", $hex);
...
/*  Google fonts
/* ------------------------------------ */
if ( ! function_exists( 'enspire_enqueue_google_fonts' ) ) {

	function enspire_enqueue_google_fonts () {
...
add_action( 'wp_enqueue_scripts', 'enspire_enqueue_google_fonts' );


/*  Dynamic css output
/* ------------------------------------ */
if ( ! function_exists( 'enspire_dynamic_css' ) ) {

	function enspire_dynamic_css() {
		if ( get_theme_mod('dynamic-styles', 'on') == 'on' ) {
		
			// rgb values
			$color_1 = get_theme_mod('color-1');
			$color_1_rgb = enspire_hex2rgb($color_1);
...
.wp-pagenavi span.current { border-bottom-color: '.esc_attr( get_theme_mod('color-1') ).'!important; }					
			...
			if ( get_theme_mod('color-logo','#1db954') != '#1db954' ) {
				$styles .= '.site-title a  { background-color: '.esc_attr( get_theme_mod('color-logo') ).'; } '."\n";
			}
			// comments bubble color
			if ( get_theme_mod('color-bubble','#1db954') != '#1db954' ) {
				$styles .= '
.post-comments,
.page-title .meta-single li.comments a  { background-color: '.esc_attr( get_theme_mod('color-bubble') ).'; }
...
.page-title .meta-single li.comments a:before { border-right-color: '.esc_attr( get_theme_mod('color-bubble') ).'; border-top-color: '.esc_attr( get_theme_mod('color-bubble') ).'; }				
			...
			wp_add_inline_style( 'enspire-style', $styles );	
		}
	}
	
}
add_action( 'wp_enqueue_scripts', 'enspire_dynamic_css' );
