...
function chili_lite_scripts() {
	 ...
	 * LOAD JS
	 */
	wp_enqueue_script( 'modernizr', get_template_directory_uri() . '/js/vendor/modernizr-2.8.3.min.js', array('jquery','masonry','jquery-ui-core'), '20151215', false );
	wp_enqueue_script( 'bootstrap', get_template_directory_uri() . '/js/bootstrap.js', array(), '20151215', true ); 
	wp_enqueue_script( 'meanmenu', get_template_directory_uri() . '/js/jquery.meanmenu.js', array('masonry'), '20151215', true );  
	wp_enqueue_script( 'chili-lite-plugins', get_template_directory_uri() . '/js/plugins.js', array(), 'nivo-slider', true );
	wp_enqueue_script( 'chili-lite-main', get_template_directory_uri() . '/js/main.js', array(), 'nivo-slider', true );
...
function chili_lite_lite_add_css() {
    ...
    <style type="text/css">
        ...
        .footer-bottom-area {
            background: <?php echo sanitize_hex_color($footer_bg_clr); ?>; 
        } 
    </style>
    <?php 
}
...
function chili_lite_comment_reform ($arg) {
 $commenter = wp_get_current_commenter();
	...
	$arg['comment_field'] = '<p class="comment-form-comment"><textarea id="comment" name="comment" cols="77" rows="8" placeholder="'. esc_attr("Сообщение", "chili-lite").'" aria-required="true"></textarea></p>';
