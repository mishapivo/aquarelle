var business_cast_upload_frame_obj;
...
( function( $ ) {
	...
	$(document).ready(function($) {
		...
		$(document).on('click', 'input.wh-image-picker', function( event ){
			...
			var businessCastImageObject = wp.media.controller.Library.extend({
				defaults : _.defaults({
					id: 'business-cast-state-insert',
			...
			business_cast_upload_frame_obj = wp.media.frames.business_cast_upload_frame_obj = wp.media({
				button: {
					text: $( this ).data( 'uploader_button_text' )
				},
				state : 'business-cast-state-insert',
				states : [
					new businessCastImageObject()
				],
				multiple: false
			});

			business_cast_upload_frame_obj.on( 'select', function() {

				var state = business_cast_upload_frame_obj.state('business-cast-state-insert');
			...
			business_cast_upload_frame_obj.open();
		});
