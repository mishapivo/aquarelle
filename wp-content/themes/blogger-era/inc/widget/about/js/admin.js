...
jQuery(function($){
  ////////////////////////////////
      ...
      jQuery(document).on('click', 'input.select-img', function( event ){
        ...
        var RTAMImage = wp.media.controller.Library.extend({
            defaults :  _.defaults({
        ...
        rtam_file_frame = wp.media.frames.rtam_file_frame = wp.media({
          button: {
        ...
        rtam_file_frame.on( 'select', function() {

          var state = rtam_file_frame.state('rtam-insert-image');
          ...
          // Show in preview
          var image_preview_wrap = $this.siblings('.raw-preview-wrap');
