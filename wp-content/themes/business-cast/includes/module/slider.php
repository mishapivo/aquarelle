...
if ( ! function_exists( 'business_cast_add_featured_slider' ) ) :
	 ...
	 * @since 1.0.0
	 */
	function business_cast_add_featured_slider() {

		$flag_apply_slider = apply_filters( 'business_cast_filter_slider_status', true );
		...
		$slider_details = apply_filters( 'business_cast_filter_slider_details', $slider_details );

		if ( empty( $slider_details ) ) {
			return;
		}

		business_cast_render_featured_slider( $slider_details );

	}

endif;

add_action( 'business_cast_action_before_content', 'business_cast_add_featured_slider', 5 );
...
add_filter( 'business_cast_filter_slider_status', 'business_cast_check_slider_status' );

if ( ! function_exists( 'business_cast_get_slider_details' ) ) :
	 ...
	 */
	function business_cast_get_slider_details( $input ) {

		$featured_slider_type           = business_cast_get_option( 'featured_slider_type' );
		$featured_slider_number         = business_cast_get_option( 'featured_slider_number' );
		$featured_slider_read_more_text = business_cast_get_option( 'featured_slider_read_more_text' );
		...
		switch ( $featured_slider_type ) {
			...
			case 'featured-page':
				...
				for ( $i = 1; $i <= $featured_slider_number ; $i++ ) {
					$id = business_cast_get_option( 'featured_slider_page_' . $i );
					if ( ! empty( $id ) ) {
						$ids[] = absint( $id );
					}
				}
				...
				if ( ! empty( $all_posts ) ) {
					...
					foreach ( $all_posts as $key => $post ) {
						...
						if ( has_post_thumbnail( $post->ID ) ) {
							...
							$slides[ $cnt ]['excerpt'] = business_cast_get_the_excerpt( apply_filters( 'business_cast_filter_slider_caption_length', 30 ), $post );
...
add_filter( 'business_cast_filter_slider_details', 'business_cast_get_slider_details' );

if ( ! function_exists( 'business_cast_render_featured_slider' ) ) :
	 ...
	 */
	function business_cast_render_featured_slider( $slider_details = array() ) {

		if ( empty( $slider_details ) ) {
			return;
		}

		$featured_slider_transition_effect   = business_cast_get_option( 'featured_slider_transition_effect' );
		$featured_slider_enable_caption      = business_cast_get_option( 'featured_slider_enable_caption' );
		$featured_slider_enable_arrow        = business_cast_get_option( 'featured_slider_enable_arrow' );
		$featured_slider_enable_pager        = business_cast_get_option( 'featured_slider_enable_pager' );
		$featured_slider_enable_autoplay     = business_cast_get_option( 'featured_slider_enable_autoplay' );
		$featured_slider_enable_overlay      = business_cast_get_option( 'featured_slider_enable_overlay' );
		$featured_slider_transition_duration = business_cast_get_option( 'featured_slider_transition_duration' );
		$featured_slider_transition_delay    = business_cast_get_option( 'featured_slider_transition_delay' );
	...
	<div id="featured-slider">
		...
		<div class="cycle-slideshow <?php echo esc_attr( $extra_slider_classes ); ?>" id="main-slider" <?php echo $slide_attributes_text; ?>>
			...
			<?php if ( $featured_slider_enable_arrow ) : ?>
				<div class="cycle-prev"><i aria-hidden="true" class="fas fa-angle-left"></i></div>
				<div class="cycle-next"><i aria-hidden="true" class="fas fa-angle-right"></i></div>
			<?php endif; ?>
