...
class RowsListControl extends BaseControl
{
    ...
    public function renderPresetsChanger()
    {
        ...
        if (isset($items['use_ajax']) && $items['use_ajax'] && isset($items['filter'])) {
            ...
            ?>
            <ul <?php $this->dataAttrs(); ?> class="list rows-list from-theme" data-ajax-data="<?php echo esc_attr($items['filter']); ?>">

            </ul>
            <script type="text/template" <?php $this->dataAttrs(); ?> id="tmpl-row-template-<?php echo esc_attr($this->id); ?>">
                <li class="item available-item {{ data.proOnly }}" data-varname="{{ data.optionsVar }}" data-id="{{ data.id }}">
        ...
        <ul <?php $this->dataAttrs(); ?> class="list rows-list from-theme">
            ...
            <?php $proOnly = isset($item['pro-only']) ? "pro-only" : ""; ?>
                ...
                <li class="item available-item <?php echo esc_attr($proOnly); ?>" data-varname="<?php echo esc_attr($optionsVar); ?>" data-id="<?php echo esc_attr($item['id']); ?>">
                    <div class="image-holder" style="background-position:center center;background-image:url()">
                    ...
                    <div title="<?php esc_attr_e('Section is already in page', 'mesmerize'); ?>" class="checked-icon"></div>
    ...
    protected function render()
    {
        ...
        <li id="accordion-section-<?php echo esc_attr($this->id); ?>" class="<?php echo esc_attr($classes); ?>">
            <div class="ope-pro-header accordion-section-title">
                <a href="<?php echo esc_url($upgrade_url); ?>" target="_blank" class="button"><?php _e("Upgrade to PRO", 'mesmerize') ?></a>
            </div>
        </li>
    ...
    protected function render()
    {
        ...
        <li id="accordion-section-<?php echo esc_attr($this->id); ?>" class="accordion-section control-section control-section-<?php echo esc_attr($this->type); ?> companion-needed-section">
            <h3 class="accordion-section-title" tabindex="0">
                ...
                <span class="screen-reader-text"><?php _e('Press return or enter to open this section', 'mesmerize'); ?></span>
            </h3>
