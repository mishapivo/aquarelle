...
add_action( 'after_setup_theme', 'mohini_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function mohini_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'mohini_construct_sidebars' );
}

if ( ! function_exists( 'mohini_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'mohini_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function mohini_woocommerce_start() { ?>
...
if ( ! function_exists( 'mohini_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'mohini_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function mohini_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'mohini_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'mohini_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function mohini_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'mohini_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'mohini_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'mohini_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function mohini_bbpress_css() {
...
if ( ! function_exists( 'mohini_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'mohini_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function mohini_buddypress_css() {
...
if ( ! function_exists( 'mohini_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'mohini_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function mohini_beaver_builder_css() {
