 ...
 */
function travel_way_sanitize_social_data( $input ){
	...
	if( !empty( $input_decoded ) ) {
		foreach ( $input_decoded as $boxes => $box ){
			foreach ( $box as $key => $value ){
				...
				elseif ( $key == 'checkbox' ){
					$input_decoded[$boxes][$key] = travel_way_sanitize_checkbox( $value );
                }
                else{
 ...
 * @return string json encoded
 */
function travel_way_sanitize_slider_data( $input ){
	$input_decoded = json_decode( $input, true );
	if( !empty( $input_decoded ) ) {
		foreach ( $input_decoded as $boxes => $box ){
			foreach ( $box as $key => $value ){
				if( $key == 'selectpage'){
					$input_decoded[$boxes][$key] = travel_way_sanitize_page( $value );
				}
...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Travel_Way_Customize_Icons_Control' )):
	/**
	 ...
	 *
	 */
	class Travel_Way_Customize_Icons_Control extends WP_Customize_Control {
            ...
            <label>
                ...
                <span class="at-customize-icons">
                    ...
                    <span class="icon-toggle">
                        <?php echo ( empty( $value )? esc_html__('Add Icon','travel-way'): esc_html__('Edit Icon','travel-way') ); ?>
                        <span class="dashicons dashicons-arrow-down"></span>
                    </span>
                    <span class="icons-list-wrapper hidden">
                        <input class="icon-search" type="text" placeholder="<?php esc_attr_e('Search Icon','travel-way')?>">
	                    <?php
...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Travel_Way_Repeater_Control' )):
	/**
	 ...
	 *
	 */
	class Travel_Way_Repeater_Control extends WP_Customize_Control {
		/**
		 * The control type.
		 *
		 * @access public
		...
		public function enqueue(){
			...
			wp_enqueue_style( 'travel-way-customizer-repeater-style', get_template_directory_uri() . '/acmethemes/customizer/customizer-repeater/customizer-repeater.css', array(), '3.3.6' );
			wp_enqueue_script( 'travel-way-customizer-repeater-script', get_template_directory_uri() . '/acmethemes/customizer/customizer-repeater/customizer-repeater.js', array('jquery', 'jquery-ui-draggable' ), '1.0.0', true  );
		}
                    ...
                    <div class="repeater-fields hidden">
								...
								switch ( $field['type'] ) {
									...
									case 'icons':
                                        ...
                                        <span class="at-customize-icons">
                                            ...
                                            <span class="icon-toggle">
                                                <?php esc_html_e('Add Icon','travel-way'); ?>
                                                <span class="dashicons dashicons-arrow-down"></span>
                                            </span>
                                            <span class="icons-list-wrapper hidden">
                                                <input class="icon-search" type="text" placeholder="<?php esc_attr_e('Search Icon','travel-way')?>">
									...
									case 'image':
                                        ...
                                        <span class="img-preview-wrap" <?php echo  $travel_way_display_none ; ?>>
                                            <img class="widefat" src="<?php echo esc_url( $new_value ); ?>" alt="<?php esc_attr_e( 'Image preview', 'travel-way' ); ?>"  />
                                        ...
                                        <input type="button" value="<?php esc_attr_e( 'Upload Image', 'travel-way' ); ?>" class="button media-image-upload" data-title="<?php esc_attr_e( 'Select Image','travel-way'); ?>" data-button="<?php esc_attr_e( 'Select Image','travel-way'); ?>"/>
                        ...
                        <div class="clearfix repeater-footer">
                            <a class="repeater-field-remove" href="#remove">
								<?php esc_html_e( 'Delete', 'travel-way' ); ?>
                            </a>
	                        <?php esc_html_e( '|', 'travel-way' ); ?>
                            <a class="repeater-field-close" href="#close">
								<?php esc_html_e( 'Close', 'travel-way' ); ?>
                            </a>
                        </div>
                    </div>
                </li>
									...
									switch ( $field['type'] ) {
										...
										case 'icons':
                                            ...
                                            <span class="at-customize-icons">
                                                ...
                                                <span class="icon-toggle">
                                                    <?php echo ( empty( $new_value )? esc_html__('Add Icon','travel-way'): esc_html__('Edit Icon','travel-way') ); ?>
                                                    <span class="dashicons dashicons-arrow-down"></span>
                                                </span>
                                                <span class="icons-list-wrapper hidden">
                                                    <input class="icon-search" type="text" placeholder="<?php esc_attr_e('Search Icon','travel-way');?>">
