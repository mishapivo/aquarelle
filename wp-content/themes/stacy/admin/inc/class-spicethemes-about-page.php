...
if (!class_exists('SpiceThemes_About_Page')) {
	class SpiceThemes_About_Page {
		...
		function __construct() {
			...
			add_action( 'stacy_info_screen', array( $this, 'stacy_recommended_actions' ), 				50 );
			add_action( 'stacy_info_screen', array( $this, 'stacy_changelog' ), 				60 );
			}
	...
	public function stacy_scripts_for_customizer() {
		...
		public function welcome_admin_notice() {
			?>
			<div class="updated notice is-dismissible stacy-notice">
				<h1><?php
				$theme_info = wp_get_theme();
				printf( esc_html__('Welcome to %1$s - Version %2$s', 'stacy'), esc_html( $theme_info->Name ), esc_html( $theme_info->Version ) ); ?>
				</h1>
				<p><?php echo sprintf( esc_html__("Welcome! Thank you for choosing SpiceThemes Stacy WordPress theme. To take full advantage of the features this theme has to offer visit our %swelcome page%s.", "stacy"), '<a href="' . esc_url( admin_url( 'themes.php?page=stacy-welcome' ) ) . '">', '</a>' ); ?></p>
				<p><a href="<?php echo esc_url( admin_url( 'themes.php?page=stacy-welcome' ) ); ?>" class="button button-blue-secondary button_stacy" style="text-decoration: none;"><?php esc_html_e('Get started with Stacy','stacy'); ?></a></p>
			</div>
			<?php
		}
	 ...
	 * 
	 */
	public function stacy_recommended_actions() {
	...
	/**
		 ...
		 */
		public function stacy_changelog() {
			global $wp_filesystem;

			?>
			<div id="changelog" class="stacy-tab-pane panel-close">
			<div class="wrap about-wrap">

				<?php //$this->intro(); ?>

				<p class="about-description"><?php esc_html_e( 'See changelog below:', 'stacy' ); ?></p>

				<?php
				$changelog_file = apply_filters( 'stacy_changelog_file', STACY_ST_TEMPLATE_DIR . '/changelog.txt' );
		...
		public function get_tabs_array() {
			...
			$tabs_array[]	= 	array(
					'link'      => 'changelog',
					'name'      => esc_html__('Changelog', 'stacy')
			);
		...
		public function get_recommended_actions() {
			...
			$act_count           = 0;
			$actions_todo = get_option( 'stacy_recommended_actions', array());
		...
		public function update_recommended_actions_watch() {
			if (isset($_POST['action_id'])) {
				...
				update_option('stacy_recommended_actions', $actions_todo);
			}
			echo json_encode(get_option('stacy_recommended_actions'));
			wp_die();
		}
...
$stacy_about_page = SpiceThemes_About_Page();