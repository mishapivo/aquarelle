...
<?php while ( have_posts() ) : the_post();
  ...
  get_template_part( 'template-parts/post','google_effect' );
  $post_title_effects = simple_days_google_post_title_effects();

  if(has_post_thumbnail()){
    get_template_part( 'template-parts/post','full_thum' );
  ...
  <div class="container m_con jc_c retaina_p0">
    <main id="post-<?php the_ID(); ?>" <?php post_class('contents post_content f_box f_col box_shadow'.$single_column); ?> itemprop="mainContentOfPage" itemscope="itemscope" itemtype="https://schema.org/Article">

      <?php

      get_template_part( 'template-parts/post','order' );
      simple_days_post_order($type = 'page' , $single_sortable , $post_title_effects , $post);
      ?>
