...
// register feminine_munk_Recent_Post widget
function feminine_munk_register_recent_post_widget(){
    register_widget( 'feminine_munk_Recent_Post' );
}

add_action( 'widgets_init', 'feminine_munk_register_recent_post_widget' );

/**
 * Adds feminine_munk_Recent_Post widget.
 */
class feminine_munk_Recent_Post extends WP_Widget {
    ...
    function __construct(){
        parent::__construct(
            'recent_post', // Base ID
            __( 'Recent Post', 'feminine-munk' ), // Name
            array( 'description' => __( 'A Recent Post Widget', 'feminine-munk' ), ) // Args
        );
    }
                    ...
                    <li>
                        <?php if ( has_post_thumbnail() && $show_thumb ) { ?>
                            <a href="<?php the_permalink(); ?>" class="post-thumbnail">
                                <?php the_post_thumbnail( 'feminine-munk-recent-post' ); ?>
                            </a>
                        <?php } ?>
                        <header class="entry-header">
                            ...
                            <?php if ( $show_date ) { ?>
                                ...
                                <span class="posted-on"><a href="<?php the_permalink(); ?>">
                                    <time datetime="<?php printf( __( 'On %1$s', 'feminine-munk' ), get_the_date('Y-m-d') ); ?>"><?php printf( __( 'On %1$s', 'feminine-munk' ), get_the_date('F j, Y') ); ?></time></a>
                                </span>
                                </div>
                            <?php } ?>
                        </header>
    ...
    public function form( $instance ){
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'num_post' ) ); ?>"><?php esc_html_e( 'Number of Posts', 'feminine-munk' ); ?></label>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_postdate' ) ); ?>"><?php esc_html_e( 'Show Post Date', 'feminine-munk' ); ?></label>
        </p>
        <?php
    }
...
} // class feminine_munk_Recent_Post 