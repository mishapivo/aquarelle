...
require_once get_template_directory() . '/inc/admin/options/featured-content.php';
...
function blogwp_register_theme_customizer( $wp_customize ) {

    if(method_exists('WP_Customize_Manager', 'add_panel')):
    $wp_customize->add_panel('blogwp_main_options_panel', array( 'title' => esc_html__('Theme Options', 'blogwp'), 'priority' => 10, ));
    endif;
    
    $wp_customize->get_section( 'title_tagline' )->panel = 'blogwp_main_options_panel';
    $wp_customize->get_section( 'title_tagline' )->priority = 20;
    $wp_customize->get_section( 'colors' )->panel = 'blogwp_main_options_panel';
    ...
    blogwp_getting_started($wp_customize);
    blogwp_header_options($wp_customize);
    blogwp_featured_content_options($wp_customize);
    blogwp_post_options($wp_customize);
    blogwp_social_profiles($wp_customize);
    blogwp_footer_options($wp_customize);
    blogwp_other_options($wp_customize);
    blogwp_recomm_plugin_options($wp_customize);
    blogwp_upgrade_to_pro($wp_customize);

}

add_action( 'customize_register', 'blogwp_register_theme_customizer' );

function blogwp_customizer_js_scripts() {
    wp_enqueue_script('blogwp-theme-customizer-js', get_template_directory_uri() . '/inc/admin/js/customizer.js', array( 'jquery', 'customize-preview' ), NULL, true);
}
add_action( 'customize_preview_init', 'blogwp_customizer_js_scripts' );