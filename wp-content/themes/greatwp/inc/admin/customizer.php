...
function greatwp_register_theme_customizer( $wp_customize ) {

    if(method_exists('WP_Customize_Manager', 'add_panel')):
    $wp_customize->add_panel('greatwp_main_options_panel', array( 'title' => esc_html__('Theme Options', 'greatwp'), 'priority' => 10, ));
    endif;
    
    $wp_customize->get_section( 'title_tagline' )->panel = 'greatwp_main_options_panel';
    $wp_customize->get_section( 'title_tagline' )->priority = 20;
    $wp_customize->get_section( 'colors' )->panel = 'greatwp_main_options_panel';
    ...
    greatwp_getting_started($wp_customize);
    greatwp_header_options($wp_customize);
    greatwp_post_options($wp_customize);
    greatwp_social_profiles($wp_customize);
    greatwp_footer_options($wp_customize);
    greatwp_other_options($wp_customize);
    greatwp_recomm_plugin_options($wp_customize);
    greatwp_upgrade_to_pro($wp_customize);

}

add_action( 'customize_register', 'greatwp_register_theme_customizer' );

function greatwp_customizer_js_scripts() {
    wp_enqueue_script('greatwp-theme-customizer-js', get_template_directory_uri() . '/inc/admin/js/customizer.js', array( 'jquery', 'customize-preview' ), NULL, true);
}
add_action( 'customize_preview_init', 'greatwp_customizer_js_scripts' );