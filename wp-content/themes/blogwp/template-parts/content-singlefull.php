                ...
                if ( !(blogwp_get_option('hide_thumbnail')) ) {
                    if ( !(blogwp_get_option('hide_thumbnail_single')) ) {
                        if ( blogwp_get_option('thumbnail_link') == 'no' ) {
                            the_post_thumbnail('blogwp-large-image', array('class' => 'blogwp-post-thumbnail-single'));
                        } else { ?>
                            <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'blogwp' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_post_thumbnail('blogwp-large-image', array('class' => 'blogwp-post-thumbnail-single')); ?></a>
                <?php   }
                    }
                }
            }
