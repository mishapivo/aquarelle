 ...
 * @since 1.0.0
 */
class Galway_Lite_Widget_Base extends WP_Widget
{
    ...
    private function render_field($key, $field, $instance)
    {
        ...
        switch ($field_type) {
            ...
            case 'image':
                ...
                <div>
                    ...
                    <input type="button" class="select-img button button-primary"
                           value="<?php esc_attr_e('Upload', 'galway-lite'); ?>"
                           data-uploader_title="<?php esc_attr_e('Select Image', 'galway-lite'); ?>"
                           data-uploader_button_text="<?php esc_attr_e('Choose Image', 'galway-lite'); ?>"/>
                    <?php
                    $image_status = false;
                    if (!empty($value)) {
                        $image_status = true;
                    ...
                    ?>
                    <input type="button" value="<?php echo esc_attr_x('X', 'Remove', 'galway-lite'); ?>"
                           class="button button-secondary btn-image-remove"
                           style="<?php echo esc_attr($remove_button_style); ?>"/>
                    <input type="hidden" class="img" name="<?php echo esc_attr($this->get_field_name($key)); ?>"
                           id="<?php echo esc_attr($this->get_field_id($key)); ?>"
                           value="<?php echo esc_attr($value); ?>"/>
            ...
            case 'message':
                ...
                ?>
                <div class="widefat field-message <?php echo esc_attr($field['class']); ?>"
                    ...
                    <?php if (!empty($field['options'])) : ?>
                        <?php foreach ($field['options'] as $k => $v) : ?>
                            <label for="<?php echo esc_attr($this->get_field_id($key) . '-' . $k); ?>">
                                <input type="radio" name="<?php echo esc_attr($this->get_field_name($key)); ?>"
                                       id="<?php echo esc_attr($this->get_field_id($key) . '-' . $k); ?>"
                    ...
                    >
                        <?php if (!empty($field['options'])) : ?>
                            <?php foreach ($field['options'] as $option_key => $label) : ?>
                                <option
                                        value="<?php echo esc_attr($option_key); ?>" <?php selected($option_key, $value); ?>><?php echo esc_html($label); ?></option>
                            <?php endforeach; ?>
                        <?php endif; ?>
