                ...
                if ( !(clean_grid_get_option('hide_thumbnail')) ) {
                    if ( !(clean_grid_get_option('hide_thumbnail_single')) ) {
                        if ( clean_grid_get_option('thumbnail_link') == 'no' ) {
                            the_post_thumbnail('clean-grid-large-image', array('class' => 'clean-grid-post-thumbnail-single'));
                        } else { ?>
                            <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'clean-grid' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_post_thumbnail('clean-grid-large-image', array('class' => 'clean-grid-post-thumbnail-single')); ?></a>
                <?php   }
                    }
                }
            }
