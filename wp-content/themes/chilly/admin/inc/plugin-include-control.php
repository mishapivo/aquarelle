...
if (class_exists('WP_Customize_Control') && !class_exists('Chilly_Plugin_Install_Control')) {
	...
	class Chilly_Plugin_Install_Control extends WP_Customize_Control {
		...
		public  function enqueue() {
			...
			wp_enqueue_script('chilly-companion-install', CHILLY_ADMIN_URI . '/assets/js/plugin-install.js', array('jquery'));
		...
		protected function render_content() {
			...
			$path = $chilly_about_page->get_plugin_basename_from_slug($this->slug);
			if($chilly_about_page->is_plugin_active($path)){
				return;
			}
			?>
