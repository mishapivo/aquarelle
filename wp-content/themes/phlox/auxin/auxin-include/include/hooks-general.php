...
function auxin_modern_comment_form( $comment_form, $params ){
    ...
    $comment_form['comment_field'] = '<div class="aux-input-group"><textarea name="comment" id="comment" cols="45" rows="8" ' . ( $req ? "aria-required='true' required" : "" ) .' ></textarea><label for="comment">'. esc_attr__('Comment' , 'phlox' ). '</label><div class="aux-modern-form-border"></div></div>';

    $comment_form['fields']['author'] = '<div class="aux-input-group"><input type="text"  name="author" id="author" value="'. esc_attr( $commenter['comment_author'] ). '" size="30" '. ( $req ? "aria-required='true' required" : "" ) .' /><label for="author">' . $author_label . '</label><div class="aux-modern-form-border"></div></div>';

    $comment_form['fields']['email'] = '<div class="aux-input-group"><input type="email" name="email"  id="email" value="'. esc_attr( $commenter['comment_author_email'] ). '" size="30" ' . ( $req ? "aria-required='true' required" : "" ) .' /><label for="email">' . $email_label . '</label><div class="aux-modern-form-border"></div></div>';
    $comment_form['fields']['url']   = '<div class="aux-input-group"><input type="url" name="url" id="url" value="'. esc_url( $commenter['comment_author_url'] ). '" size="30" /><label for="url">' . esc_html__( 'Website', 'phlox' ) . '</label><div class="aux-modern-form-border"></div></div>';

    $comment_form['title_reply'] = '<span>' . esc_html__('Add a Comment', 'phlox' ) . '</span>';
    $comment_form['comment_notes_before'] = '<p class="comment-notes">' . __( 'Your email address will not be published. Required fields are marked', 'phlox' ) .'<span class="aux-required-field">*</span></p>';
