...
class Auxin_Welcome_Base {
    ...
    public function get_admin_menu_args(){
        $menu_name = AUXIN_NO_BRAND ? __( 'Theme Panel', 'phlox') : THEME_NAME_I18N;

        return apply_filters( 'auxin_admin_welcome_menu_args', array(
            'name'          => $menu_name,
            'title'         => __('Welcome', 'phlox'),
    ...
    protected function the_header( $type ){
        ...
        $welcome_page_title  = AUXIN_NO_BRAND ? __('Welcome','phlox') : sprintf( __( 'Welcome to %s Theme', 'phlox' ) . '</strong>',  '<strong>'. THEME_NAME_I18N );
        /**
        ...
        */
        ...
        <div class="aux-welcome-header">
            ...
            <div class="aux-welcome-subtitle"><?php printf( esc_html__('Version %s', 'phlox'), THEME_VERSION );; echo esc_html( $welcome_description ); ?></div>
            <a class="aux-welcome-theme-badge" href="<?php echo $this->get_tab_link(); ?>"></a>
        </div>
        <?php
    }
