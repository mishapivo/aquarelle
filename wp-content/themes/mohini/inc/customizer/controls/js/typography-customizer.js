( function( api ) {
	...
	api.controlConstructor['mohini-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.mohini-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the mohini_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'mohini_get_all_google_fonts_ajax',
								mohini_customize_nonce: mohini_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.mohini-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.mohini-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.mohini-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.mohini-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.mohini-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.mohini-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.mohini-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.mohini-font-variant' );
			...
			control.container.on( 'change', '.mohini-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.mohini-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.mohini-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.mohini-font-family select' ).selectWoo();
	jQuery( '.mohini-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".mohini-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.mohini-font-variant' ).hide();
		}
	});
