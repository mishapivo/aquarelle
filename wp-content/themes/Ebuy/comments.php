...
<?php if ( have_comments() ) : ?>
	<h3 id="comments"><?php comments_number('Нет комментариев', '1 комментарий',  'Комментариев: %' );?> </h3>
...
<p>Вы должны <a href="<?php echo wp_login_url( get_permalink() ); ?>">авторизоваться</a>, чтобы оставить комментарий.</p>
<?php else : ?>
...
<label for="email">Почта <small><?php if ($req) echo "(Обязательно)"; ?></small></label>
<input type="text" name="email" id="email" value="<?php echo esc_attr($comment_author_email); ?>" size="22" tabindex="2" <?php if ($req) echo "aria-required='true'"; ?> />

<label for="url">Вэб-сайт</label>
