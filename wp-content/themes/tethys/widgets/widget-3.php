...
class tethys_widget_3 extends WP_Widget {

/*  Widget #3 Setup  */

	public function __construct() {
		parent::__construct(false, $name = esc_html__('#3 Widget [tethys]', 'tethys' ), array(
			'description' => esc_html__('Widget #3 for the Homepage', 'tethys' )
		));
	}
						...
						<div class="space-news-widget-3 case-15 relative">
							<div class="space-news-widget-3-items box-100 relative">
								...
								<div class="space-news-widget-3-item box-100 relative">
									<?php
										$widget_3_big = wp_get_attachment_image_src(get_post_thumbnail_id(), 'tethys-medium-img'); 
										$widget_3_big_alt = get_post_meta(get_post_thumbnail_id(), '_wp_attachment_image_alt', true);
										if ($widget_3_big) { ?>
									<div class="space-news-widget-3-item-img relative">
										<a href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
											<div class="space-news-widget-3-item-img-ins relative">
												<img src="<?php echo esc_url($widget_3_big[0]); ?>" alt="<?php echo esc_attr($widget_3_big_alt); ?>">
												<div class="space-gradient-overlay white absolute"></div>
												<div class="space-news-widget-3-item-img-date absolute"><i class="fa fa-calendar-o" aria-hidden="true"></i> <?php if( get_theme_mod('tethys_time_ago_format') ){ ?><?php printf( esc_html_x( '%s ago', '%s = human-readable time difference', 'tethys' ), human_time_diff( get_the_time( 'U' ), current_time( 'timestamp' ) ) ); ?><?php } else { echo get_the_date(); } ?></div>
											</div>
										</a>
										<div class="space-news-widget-3-category absolute"><?php foreach( $terms as $term ){ echo esc_html($term->name); } ?></div>
									</div>
									<?php } ?>
									<div class="space-news-widget-3-item-meta relative">
										<div class="space-news-widget-3-item-meta-title relative">
											...
											<?php if ( has_post_format( 'gallery' )) { ?>
												<div class="space-post-format relative"><i class="fa fa-picture-o" aria-hidden="true"></i></div>
											<?php } ?>
											<div class="space-news-widget-3-item-meta-excerpt relative">
												<?php the_excerpt(); ?>
											</div>
										</div>
									</div>
...
add_action( 'widgets_init', 'tethys_widget_3' );

function tethys_widget_3() {
	register_widget( 'tethys_widget_3' );
}