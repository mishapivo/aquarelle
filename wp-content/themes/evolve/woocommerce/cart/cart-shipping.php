...
<tr class="woocommerce-shipping-totals shipping">
    ...
    <td data-title="<?php echo esc_attr( $package_name ); ?>">
		<?php if ( $available_methods ) : ?>
            <ul id="shipping_method" class="woocommerce-shipping-methods mb-0">
				<?php foreach ( $available_methods as $method ) : ?>
                    <li>
						<?php
						if ( 1 < count( $available_methods ) ) {
							printf( '<input type="radio" name="shipping_method[%1$d]" data-index="%1$d" id="shipping_method_%1$d_%2$s" value="%3$s" class="shipping_method mr-2" %4$s />', $index, esc_attr( sanitize_title( $method->id ) ), esc_attr( $method->id ), checked( $method->id, $chosen_method, false ) ); // WPCS: XSS ok.
						} else {
							printf( '<input type="hidden" name="shipping_method[%1$d]" data-index="%1$d" id="shipping_method_%1$d_%2$s" value="%3$s" class="shipping_method mr-2" />', $index, esc_attr( sanitize_title( $method->id ) ), esc_attr( $method->id ) ); // WPCS: XSS ok.
						}
			...
			<?php if ( is_cart() ) : ?>
                <p class="woocommerce-shipping-destination text-right mt-3">
					<?php
        ...
        elseif ( ! is_cart() ) :
			...
			$calculator_text = __( 'Enter a different address', 'evolve' );
		endif;
		?>
