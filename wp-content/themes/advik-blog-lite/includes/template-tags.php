 ...
 * @since 1.0
 */
function advik_blog_lite_content_more() {	
	/* translators: link read more. */
	$text = wp_kses_post( sprintf( esc_html__( 'Continue reading &#10142; %s', 'advik-blog-lite' ), '<span class="screen-reader-text">' . get_the_title() . '</span>' ) );
	$more = sprintf( '<div class="link-more"><a href="%s#more-%d" class="more-link">%s</a></div>', esc_url( get_permalink() ), get_the_ID(), $text );
	if ( !is_admin() ) {
		return $more;
	}
}
add_filter( 'the_content_more_link', 'advik_blog_lite_content_more' );
 ...
 * @since 1.0
 */
function advik_blog_lite_excerpt_more_link( $excerpt ) {
	...
	if ( is_home() || is_front_page() ) { 	
		$show_readmore 	= advik_blog_lite_get_theme_mod( 'blog_show_readmore' );
	} elseif( is_category() || is_archive() ) {
		$show_readmore 	= advik_blog_lite_get_theme_mod( 'cat_show_readmore' );
	}else{
		$show_readmore 	= advik_blog_lite_get_theme_mod( 'blog_show_readmore' );
	}
	
	if( !empty($show_readmore) ){
		$excerpt 	.= advik_blog_lite_content_more();	
	}
    
    return $excerpt;
}
add_filter( 'the_excerpt', 'advik_blog_lite_excerpt_more_link', 21 );

/**
 * Prints HTML with meta information for the current post-date/time and categories, tags..
 */
function advik_blog_lite_cat_posted_on( $meta = array() ) {
	...
	if( is_home() || is_front_page() || is_search()) {
		$category 	= advik_blog_lite_get_theme_mod( 'blog_show_cat' );
	} elseif( is_category() || is_archive() || is_tag() || is_author() ) {
		$category 	= advik_blog_lite_get_theme_mod( 'cat_show_cat' );
	}
 ...
 * Prints HTML with meta information for the current post-date/time and categories, tags..
 */
function advik_blog_lite_posted_on( $meta = array() ) {
	...
	if( is_home() || is_front_page() || is_search()) {
		...
		$post_date 			= advik_blog_lite_get_theme_mod( 'blog_show_date' );
		$author 			= advik_blog_lite_get_theme_mod( 'blog_show_author' );			
		$blog_show_comment 	= advik_blog_lite_get_theme_mod( 'blog_show_comment' );

	} elseif( is_category() || is_archive() ||  is_author() ) {

		$post_date 			= advik_blog_lite_get_theme_mod( 'cat_show_date' );
		$author 			= advik_blog_lite_get_theme_mod( 'cat_show_author' );		
		$blog_show_comment 	= advik_blog_lite_get_theme_mod( 'cat_show_comment' );	
	...
	if ( !empty($blog_show_comment) &&  !post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( sprintf( wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'advik-blog-lite' ), array(
			'span' => array(
				'class' => array(),
			),
		) ), get_the_title() ) );
 ...
 */
function advik_blog_lite_tags_posted_on( $meta = array() ) {
	...
	if( is_home() || is_front_page() || is_search()) {			
		$tag 				= advik_blog_lite_get_theme_mod( 'blog_show_tags' );
	} elseif( is_category() || is_archive() || is_tag() || is_author() ) {		
		$tag 				= advik_blog_lite_get_theme_mod( 'cat_show_tags' );	
	...
	if ( $tag && 'post' === get_post_type() ) {
		echo '<div class="entry-meta">';
		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'advik-blog-lite' ) );
		if ( $tags_list ) {
 ...
 * Prints HTML with meta information for the current post-date/time and categories, tags..
 */
function advik_blog_lite_latest_posted_on( $meta = array() ) {
 ...
 */
function advik_blog_lite_entry_footer() {
	edit_post_link(
		sprintf(
			/* translators: %s: Name of current post */
			esc_html__( 'Edit %s', 'advik-blog-lite' ),
 ...
 * @return mixed
 */
function advik_blog_lite_tag_cloud_args( $args ) {
...
add_filter( 'widget_tag_cloud_args', 'advik_blog_lite_tag_cloud_args' );