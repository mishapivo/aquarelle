					...
					switch ( $column_id ) {
						case 'quantity':
							...
							if ( ! $grouped_product_child->is_purchasable() || $grouped_product_child->has_options() || ! $grouped_product_child->is_in_stock() ) {
								...
								echo '<div class="custom-control custom-checkbox float-right"><input type="checkbox" name="' . esc_attr( 'quantity[' . $grouped_product_child->get_id() . ']' ) . '" value="1" class="wc-grouped-product-add-to-cart-checkbox custom-control-input" id="custom-check" /><label class="custom-control-label" for="custom-check"></label></div>';
							} else {
