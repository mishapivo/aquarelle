<?php if( get_theme_mod( 'slider_display_option', $default = false ) ) : ?>
  ...
  <?php if ( $query->have_posts() && $number_of_posts ) : ?>
    ...
    <?php
      $layout = get_theme_mod( 'lifestyle_magazine_slider_layouts', 'one' );
      if( $layout == 'one' ) {
              ...
              <?php $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'large' ); ?>
                  ...
                  <div class="banner-news-caption">
                    ...
                    <?php if( in_array( 'readmore', $post_details ) ) : ?>
                      <a href="<?php echo esc_url( get_the_permalink( $post->ID ) ); ?>" class="readmore"><?php esc_attr_e( 'Read More', 'lifestyle-press' ); ?></a>
                    <?php endif; ?>
                  </div> 
              </div>
            </div>
