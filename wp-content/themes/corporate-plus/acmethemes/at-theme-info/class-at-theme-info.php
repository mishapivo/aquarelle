...
if ( ! class_exists( 'Corporate_Plus_Theme_Info' ) ) {
    class Corporate_Plus_Theme_Info{
        ...
        public function at_theme_info_welcome_admin_notice() {
            ...
            echo ( '<p>' . sprintf( __('Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.','corporate-plus'), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-info' ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-info' ) ) . '" class="button" style="text-decoration: none;">' . sprintf( __('Get started with %s','corporate-plus'), $this->theme_name ) . '</a></p>' );
            echo '</div>';
        }
...
$config = array(
    ...
    'recommended_actions'        => array(
        'demo-content' => array(
            'title' => esc_html__( 'Install Demo Setup','corporate-plus' ),
            'desc' => sprintf( esc_html__( 'For the demo content installation, install the Acme Demo Setup plugin, download demo content from %1$s here %2$s. Then go to "Acme Demo Setup" and import demo. ', 'corporate-plus' ), '<a target="_blank" href="http://www.doc.acmethemes.com/corporate-plus/#DummyData" >','</a>' ),
            'id' => 'acme-demo-setup',
...
return new Corporate_Plus_Theme_Info( $config );