...
if ( ! class_exists( 'Blog_Kit_About' ) ) {
	...
	class Blog_Kit_About {
		...
		public function setup_config() {
			...
			$this->logo_url      = isset( $this->config['logo_url'] ) ? $this->config['logo_url'] : get_template_directory_uri() . '/inc/theme-info/images/cap-logo.png';
			$this->logo_link     = isset( $this->config['logo_link'] ) ? $this->config['logo_link'] : 'https://wpcapsules.com/';
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_enqueue_style( 'blog-kit-about', get_template_directory_uri() . '/inc/theme-info/css/about.css', array(), '2.0.2' );
				wp_enqueue_script( 'blog-kit-about', get_template_directory_uri() . '/inc/theme-info/js/about.js', array( 'jquery' ), '2.0.2' );
				$js_vars = array(
		...
		public function free_pro() {
		    ...
		    if ( ! empty( $free_pro ) ) {
		        ...
		        if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
		            ...
		            echo '<td></td>';
		            echo '<td colspan="2"><a href="https://wpcapsules.com/downloads/blog-kit-pro/" target="_blank" class="button button-primary button-hero">Blog Kit Pro</a></td>';
		            echo '</tr>';
