...
if ( ! function_exists( 'the_conference_customizer_demo_content' ) ) :
	...
	function the_conference_customizer_demo_content( $wp_customize ) {
	    ...
	    $demo_content_description = sprintf( '%1$s<a class="documentation" href="%2$s" target="_blank">%3$s</a>', esc_html__( 'Conference comes with demo content import feature. You can import the demo content with just one click. For step-by-step video tutorial, ', 'the-conference' ), esc_url( 'https://raratheme.com/blog/import-demo-content-rara-themes/' ), esc_html__( 'Click here', 'the-conference' ) );
		...
		$theme_demo_content_desc .= '<span class="sticky_info_row download-link"><label class="row-element">' . __( 'Download Demo Content', 'the-conference' ) . ': </label><a href="' . esc_url( 'https://raratheme.com/documentation/the-conference/' ) . '" target="_blank">' . __( 'Click here', 'the-conference' ) . '</a></span><br />';
