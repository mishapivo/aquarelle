...
class oxane_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'oxane_rp', // Base ID
			__('Oxane Recent Posts with Thumbnails', 'oxane'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'oxane' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
		...
		<?php
         ...
         ?>
		         ...
		         <div class='rp-date'><?php date_i18n(__('M j, Y','oxane')); ?></div>
		         </li>      
		...
		?>
		      ...
		      <?php _e('Oops, there are no posts.','oxane'); ?>
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'oxane' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'oxane' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','oxane' ); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'oxane' ); ?></label>
...
add_action( 'widgets_init', 'oxane_register_widget' );  
function oxane_register_widget() {  
    register_widget( 'oxane_Recent_Posts' );  
}  
