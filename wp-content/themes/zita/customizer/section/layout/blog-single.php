...
// Remove Feature image padding
    ...
    // enable share icon
    $wp_customize->add_setting( 'zita_single_share_icon', array(
                'default'           => true,
                'sanitize_callback' => 'zita_sanitize_checkbox',
            ) );
    $wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'zita_single_share_icon', array(
                'label'       => esc_html__('Enable Share Icon', 'zita'),
                'type'        => 'checkbox',
                'section'     => 'zita-blog-single',
                'settings'    => 'zita_single_share_icon',
                'priority'    =>8,
            ) ) );
    // DISPALY AUTOR BIO
...
$wp_customize->add_control(new Zita_Misc_Control( $wp_customize, 'zita_blog_learn_more',
        ...
        'section'     => 'zita-blog-single',
        'type'        => 'custom_message',
        'description' => sprintf( wp_kses(__( 'To know more Go with this <a target="_blank" href="%s">Doc</a> !', 'zita' ), array(  'a' => array( 'href' => array(),'target' => array() ) ) ), esc_url('https://wpzita.com/docs/single-post/')),
        'priority'   =>30,
    )));