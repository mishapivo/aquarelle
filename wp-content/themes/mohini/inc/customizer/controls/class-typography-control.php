...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Mohini_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Mohini_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'mohini-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'mohini-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), MOHINI_VERSION, true );
			wp_enqueue_style( 'mohini-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), MOHINI_VERSION );

			wp_enqueue_script( 'mohini-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'mohini-typography-selectWoo' ), MOHINI_VERSION, true );
			wp_enqueue_style( 'mohini-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), MOHINI_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'mohini_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'mohini' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'mohini' );
			$this->json[ 'google_fonts' ] = apply_filters( 'mohini_typography_customize_list', mohini_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = mohini_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'mohini' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'mohini' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'mohini' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'mohini' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="mohini-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="mohini-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="mohini-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="mohini-font-transform">
					<label>
